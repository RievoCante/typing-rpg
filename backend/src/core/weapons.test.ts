...
describe('isWeaponId', () => {
  it('accepts every pool id', () => {
    for (const id of WEAPON_IDS) expect(isWeaponId(id)).toBe(true);
  });
...
describe('mergeUnlocked', () => {
  it('unions current + incoming, deduped, filtered, in canonical order', () => {
    const merged = mergeUnlocked(
      ['dragonfang'],
      ['iron-sword', 'dragonfang', 'bogus', 'wooden-club']
    );
  ...
  it('ignores non-string / invalid incoming entries', () => {
    expect(
      mergeUnlocked([], ['iron-sword', 123 as unknown, null as unknown, ''])
    ).toEqual(['iron-sword']);
  });
...
describe('unlockSchema', () => {
  it('accepts a non-empty id array within the pool size', () => {
    expect(unlockSchema.safeParse({ weaponIds: ['iron-sword'] }).success).toBe(
      true
    );
  });
...
describe('selectSchema', () => {
  it('accepts a string id or null', () => {
    expect(selectSchema.safeParse({ weaponId: 'iron-sword' }).success).toBe(
      true
    );
