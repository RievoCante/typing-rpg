...
const ID_SET = new Set<string>(WEAPON_IDS);
...
export function parseUnlocked(raw: string | null | undefined): WeaponId[] {
  if (!raw) return [];
  try {
...
export function mergeUnlocked(
  ...
  const set = new Set<WeaponId>(current.filter(isWeaponId));
  for (const id of incoming) if (isWeaponId(id)) set.add(id);
