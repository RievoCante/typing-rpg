...
export const getLevelLeaderboard = async (c) => {
...
export const getTodayDailyWpmLeaderboard = async (c) => {
    const url = new URL(c.req.url);
    const limitParam = url.searchParams.get("limit");
    const offsetParam = url.searchParams.get("offset");
    let limit = Number.parseInt(limitParam || "50", 10);
    ...
    try {
        ...
        const rows = await db
            ...
            .where(and(eq(gameSessions.mode, "daily"), gte(gameSessions.createdAt, dayStart), lt(gameSessions.createdAt, dayEnd)))
