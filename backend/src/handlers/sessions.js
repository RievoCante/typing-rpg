...
// POST /api/sessions
export const createSession = async (c) => {
    ...
    const { mode, wpm, totalWords, correctWords, incorrectWords, difficulty, rawWpm, accuracy, consistency, correctChars, incorrectChars, extraChars, missedChars, durationSeconds, afkSeconds, chartData, } = parsed.data;
    const db = c.get('db');
    try {
        ...
        const sessionWithXp = { ...inserted, xpDelta };
...
export const getDailyStatus = async (c) => {
    const auth = getAuth(c);
    const userId = auth?.userId;
    if (!userId)
        return c.json({ error: 'Unauthorized' }, 401);
