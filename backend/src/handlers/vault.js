...
import { parseUnlocked, mergeUnlocked, isWeaponId, unlockSchema, selectSchema, } from '../core/weapons';
// GET /me/vault — the signed-in user's unlocked weapons + selected loadout.
export const getVault = async (c) => {
...
export const unlockWeapons = async (c) => {
    const auth = getAuth(c);
    if (!auth?.userId)
        return jsonError(c, 401, 'Unauthorized');
    let body;
    ...
    if (!user)
        return jsonError(c, 404, 'User not found');
    const merged = mergeUnlocked(parseUnlocked(user.unlockedWeapons), parsed.data.weaponIds);
    await db
        .update(users)
...
export const selectLoadout = async (c) => {
    const auth = getAuth(c);
    if (!auth?.userId)
        return jsonError(c, 401, 'Unauthorized');
    let body;
