...
import { getLevelLeaderboard, getTodayDailyWpmLeaderboard, } from "./handlers/leaderboard";
...
const keyFn = (c) => getAuth(c)?.userId ?? c.req.header("cf-connecting-ip") ?? "anon";
...
const worker = {
    async fetch(req, env, ctx) {
        ...
        if (url.pathname.startsWith('/raid/') &&
            url.pathname.length === 12 &&
            req.headers.get('Upgrade') === 'websocket') {
            const authResult = await validateRaidWsAuth(url, env);
            if (!authResult.ok) {
