...
export class RaidRoom extends DurableObject {
    ...
    async ensureInitialized() {
        if (this.initialized)
            return;
        try {
            const persistedRoomId = await this.ctx.storage.get('roomId');
            const persistedDbCreated = await this.ctx.storage.get('dbRoomCreated');
            if (persistedRoomId) {
    ...
    handlePlayerJoin(ws, data, characterConfig = null) {
        if (this.state.phase !== 'lobby') {
            return;
        }
    ...
    async createRoomInDb(hostId, hostUsername) {
        try {
            const env = this.env;
            if (!env.DB) {
    ...
    async syncPlayerCountToDb(count, status, hostId, hostUsername) {
        try {
            const env = this.env;
            if (!env.DB)
                return;
            ...
            if (hostUsername)
                updates.hostUsername = hostUsername;
    ...
    handleStartGame(ws) {
        ...
        if (playerCount < MIN_PLAYERS) {
            try {
                ws.send(JSON.stringify({ type: 'error', message: `Need at least ${MIN_PLAYERS} players to start` }));
            }
            catch {
                // Best-effort
            }
        ...
        if (playerCount > MAX_PLAYERS) {
            try {
                ws.send(JSON.stringify({ type: 'error', message: `Max ${MAX_PLAYERS} players allowed` }));
            }
            catch {
                // Best-effort
            }
    ...
    async persistSessionAndAwardXp(status) {
            ...
            for (const p of playerStates) {
                ...
                try {
                    const [u] = await db
                        ...
                        .limit(1);
                    if (!u)
                        continue; // No user row (e.g. account deleted) — skip XP grant.
    ...
    broadcastRoomState(extra) {
        ...
        if (extra?.result)
            state.result = extra.result;
        if (extra?.stats)
            state.stats = extra.stats;
        for (const [ws] of this.state.players) {
            try {
