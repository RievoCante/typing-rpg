...
export const useDailyProgress = () => {
  const [dailyState, setDailyState] =
    useState<DailyState>(getDefaultDailyState);
  ...
  useEffect(() => {
    ...
    if (savedState) {
      try {
        ...
        console.error(
          'Failed to parse daily progress from localStorage:',
          error
        );
        setDailyState(getDefaultDailyState());
      }
    } else {
      // If there is no saved state, set the state to the default state
  ...
  const completeCurrentQuote = useCallback((wpm: number, attempts: number) => {
    setDailyState(prev => {
      ...
      const alreadyCompleted = prev.quoteStats.some(
        stat => stat.difficulty === prev.currentQuote
      );

      if (alreadyCompleted) {
        console.warn(
          `Quote difficulty ${prev.currentQuote} already completed, skipping duplicate`
        );
      ...
      const newState = {
        ...
        isCompleted,
        lastCompletionDate: isCompleted
          ? getUTCDateString()
      ...
      console.log(
        `Daily progress updated: ${prev.completedQuotes} -> ${newCompletedQuotes} (${prev.currentQuote} -> ${newCurrentQuote})`
      );
  ...
  const getAverageWPM = useCallback((): number => {
    ...
    const totalWPM = dailyState.quoteStats.reduce(
  ...
  return {
    ...
    isCompletedToday:
      ...
      dailyState.lastCompletionDate === getUTCDateString(), // Completed today
